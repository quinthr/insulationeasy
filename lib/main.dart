...
Future<String> uploadChecklist(String formId) async {
  ...
  for(var i=0; i<dataList.length; i++){
    ...
    await http.post(url, body: checklist, headers: {
...
Future<String> uploadImage(String formId) async {
  ...
  for(var i=0; i<dataList.length; i++){
    ...
    await http.post(url, body: image, headers: {
      "x-access-token": "SW5zdWxhdGlvbkVhc3lBdXN0cmFsaWEmU291bmRwcm9vZmluZ1Byb2R1Y3RzQXVzdHJhbGlh",
      "Content-Type": "application/json"});
...
Future<String> uploadHazards(String formId) async {
  ...
  for(var i=0; i<dataList.length; i++){
    ...
    await http.post(url, body: hazards, headers: {
      "x-access-token": "SW5zdWxhdGlvbkVhc3lBdXN0cmFsaWEmU291bmRwcm9vZmluZ1Byb2R1Y3RzQXVzdHJhbGlh",
      "Content-Type": "application/json"});
...
Future<String> uploadSignature(String formId) async {
  ...
  for(var i=0; i<dataList.length; i++){
    ...
    await http.post(url, body: signatures, headers: {
      "x-access-token": "SW5zdWxhdGlvbkVhc3lBdXN0cmFsaWEmU291bmRwcm9vZmluZ1Byb2R1Y3RzQXVzdHJhbGlh",
      "Content-Type": "application/json"});
  }
}
...
Future<dynamic> uploadFormEntries() async{
  ...
  for(var i=0; i<dataList.length; i++){
    ...
    await http.post(url, body: entry, headers: {
      "x-access-token": "SW5zdWxhdGlvbkVhc3lBdXN0cmFsaWEmU291bmRwcm9vZmluZ1Byb2R1Y3RzQXVzdHJhbGlh",
    "Content-Type": "application/json"});
    await InstallationFormEntryDB.SetUploaded(dataList[i]['formId']);
    await uploadChecklist(dataList[i]['formId']);
    await uploadImage(dataList[i]['formId']);
    await uploadHazards(dataList[i]['formId']);
    await uploadSignature(dataList[i]['formId']);
    Future.delayed(const Duration(seconds: 2), () {
      sendToEmail(dataList[i]['formId']);
    });
  }
  return 0;
}
...
class _MyHomePageState extends State<MyHomePage> {
  _MyHomePageState() {
    ...
    checkInternetConnection().then((val) => setState(() {
      if(val == true){
        uploadFormEntries().then((val2) => updateFormEntries());
      }
    }));
  }
  @override
